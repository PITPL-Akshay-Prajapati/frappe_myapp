...
def changeLogo_patch():
	...
	update_field_label()
	if cint(get_frappe_version()) >= 13 and not frappe.db.get_single_value('changeLogo Setting', 'ignore_onboard_changeLogo'):
...
def boot_session(bootinfo):
	...
	if frappe.session['user']!='Guest':
		...
		bootinfo.changeLogo = frappe.get_doc("changeLogo Setting","changeLogo Setting")

@frappe.whitelist()
def ignore_update_popup():
	if not frappe.db.get_single_value('changeLogo Setting', 'disable_new_update_popup'):
